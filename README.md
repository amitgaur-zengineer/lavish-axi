...
- **Session index** - Loading the server's root URL in a browser on the same machine (loopback only) shows every open session with a file path, status, pending-feedback count, active listener, and an Open link, newest-first and paginated 10 per page with Previous/Next (`?page=N`). Sessions ended within the last 7 days (capped at 20) render greyed underneath with no link, since reopening a user-ended session is `--reopen`, not a click. With no sessions at all, the page shows the original landing card instead. The index is unavailable to any non-loopback caller - including a Tailscale peer - and to any request that traversed a reverse proxy (identified by a present `X-Forwarded-Host`), which sees the landing card instead.
